...
sppEquiv2$Species <- sub("(.*)", "\\*\\1\\*",  sppEquiv$Species)  ## for italics
...
```{r load-SpaDES-Biomass-speciesData, eval=FALSE}
if (!require(Require)) {
...
```{r module usage example2-Biomass-speciesData, eval=FALSE}
...
```{r fig-Biomass-speciesDataOutPlots, echo = FALSE, eval = TRUE, fig.show='hold', out.width = "50%", fig.cap = "(ref:Biomass-speciesData) automatically generates a plot of species dominance and number of presences in the study area, when `.plotInitialTime = 1` is passed as an argument."}
